...
// Complete JavaScript Application (MERGED)
// ===============================================
  ...
  try {
    ...
    function rotateEarth() {
      if (autoRotate && earth) {
        ...
        earth.setCenter([currentCenter[0], currentCenter[1] + 0.05]);
      }
    ...
    rotateBtn.addEventListener("click", function () {
      ...
      this.style.background = autoRotate
        ? "rgba(255, 107, 107, 0.3)"
        : "rgba(255, 255, 255, 0.2)";
    });

    const earthSide = document.getElementById("earth-side");
    if (earthSide) {
      earthSide.appendChild(rotateBtn);
    }

    setTimeout(() => rotateEarth(), 100);
...
function addGlobeControls() {
  ...
  const earthSide = document.getElementById("earth-side");
  if (earthSide) {
    earthSide.appendChild(controlPanel);
  }
}
...
function initializeLeafletMap() {
  ...
  try {
    ...
    impactMap.on("click", function (e) {
      ...
      const terrain = detectTerrain(e.latlng.lat, e.latlng.lng);
      document.getElementById("terrain").value = terrain;
    });
...
function calculateImpact() {
  ...
  */
  const overlay = document.getElementById("impact-results-overlay");
  if (overlay) {
    overlay.style.display = "block";
    // Actualizar valores en overlay
    document.getElementById("overlay-energy").textContent = `${energyMT.toFixed(
      2
    )} MT`;
    document.getElementById(
      "overlay-radius"
    ).textContent = `${destructionRadius.toFixed(2)} km`;
    document.getElementById("overlay-crater").textContent =
      atmospheric.impactType === "airburst"
        ? "Airburst (no crater)"
        : `${(craterDiameter / 1000).toFixed(2)} km`;
    document.getElementById("overlay-type").textContent =
      atmospheric.impactType.charAt(0).toUpperCase() +
      atmospheric.impactType.slice(1);
  }
  ...
  drawDestructionCircles3D(destructionRadius);
  updateAnalysisPanel({
    casualties,
    economicDamage,
    energyMT,
...
function drawDestructionCircles3D(radiusKm) {
  if (!earth) return;

  // Limpiar círculos previos
  if (window.destructionCircles3D) {
    window.destructionCircles3D.forEach((circle) => earth.removeLayer(circle));
  }
  window.destructionCircles3D = [];

  const zones = [
    { radius: radiusKm, color: "#ff0000", opacity: 0.3 },
    { radius: radiusKm * 2, color: "#ff6b00", opacity: 0.2 },
    { radius: radiusKm * 3, color: "#ffd700", opacity: 0.15 },
    { radius: radiusKm * 5, color: "#4169e1", opacity: 0.1 },
  ];

  zones.forEach((zone) => {
    // Convertir radio en km a grados (aproximado)
    const radiusDeg = zone.radius / 111; // 1 grado ≈ 111 km

    const circle = WE.polygon(
      generateCirclePoints(impactLocation.lat, impactLocation.lng, radiusDeg),
      {
        color: zone.color,
        fillColor: zone.color,
    ...
    window.destructionCircles3D.push(circle);
  });
}

// 3. FUNCIÓN AUXILIAR: Generar puntos de círculo
function generateCirclePoints(lat, lng, radiusDeg, points = 64) {
  const coords = [];
  for (let i = 0; i <= points; i++) {
    const angle = (i / points) * 2 * Math.PI;
    const dx = radiusDeg * Math.cos(angle);
    const dy = radiusDeg * Math.sin(angle);
    coords.push([lat + dy, lng + dx / Math.cos((lat * Math.PI) / 180)]);
  }
  return coords;
}
...
function updateAnalysisPanel(results) {
  const analysisPanel = document.getElementById("analysis-panel");
  if (!analysisPanel) return;

  const { casualties, economicDamage, energyMT, destructionRadius } = results;

  analysisPanel.innerHTML = `
    <div class="analysis-section">
      <h4>⚡ Immediate Effects (Day 0)</h4>
      <p>The ${energyMT.toFixed(
        0
      )} megaton impact creates immediate catastrophic effects:</p>
      <ul class="effect-list">
        <li>Fireball formation exceeding ${(energyMT * 100).toFixed(
          0
        )}°C at ground zero</li>
        <li>Supersonic shockwave traveling at ${(energyMT * 0.5).toFixed(
          1
        )} km/s</li>
        <li>Ground zero vaporization within ${(destructionRadius * 0.1).toFixed(
          2
        )} km radius</li>
        <li>Seismic magnitude ${(4 + 0.67 * Math.log10(energyMT)).toFixed(
          1
        )} earthquake</li>
        <li>Thermal radiation igniting fires up to ${destructionRadius.toFixed(
          1
        )} km away</li>
      </ul>
    </div>
    ...
    <div class="analysis-section">
      <h4>🌪️ Secondary Effects (Hours to Days)</h4>
      <p>Following the initial ${destructionRadius.toFixed(
        0
      )}km destruction zone:</p>
      <ul class="effect-list">
        <li>Global atmospheric shockwave detected ${(
          destructionRadius * 10
        ).toFixed(0)}km away</li>
        <li>Ejected material raining down over ${(
          destructionRadius * 20
        ).toFixed(0)}km² area</li>
        <li>Regional firestorms affecting ${(
          casualties.total / 1000000
        ).toFixed(1)}M people</li>
        <li>Infrastructure collapse - $${(
          economicDamage.infrastructure / 1000
        ).toFixed(0)}B in damage</li>
        <li>Communication disruption across ${(destructionRadius * 5).toFixed(
          0
        )}km radius</li>
      </ul>
    </div>
    ...
    <div class="analysis-section">
      <h4>❄️ Long-term Consequences (Weeks to Months)</h4>
      <p>Lasting environmental effects from ${energyMT.toFixed(0)}MT impact:</p>
      <ul class="effect-list">
        <li>Atmospheric dust blocking ${(energyMT * 0.01).toFixed(
          1
        )}% of sunlight</li>
        <li>Temperature drop of ${(energyMT * 0.001).toFixed(
          2
        )}°C affecting agriculture</li>
        <li>${
          energyMT > 1000 ? "Severe" : "Potential"
        } impact winter lasting ${Math.floor(energyMT / 100)} months</li>
        <li>Ecosystem disruption - ${(casualties.total * 10).toFixed(
          0
        )} animals affected</li>
        <li>Global economic impact - $${(economicDamage.total / 1000).toFixed(
          0
        )}B total losses</li>
      </ul>
    </div>
...
function resetImpact() {
  // Ocultar overlay de resultados
  const overlay = document.getElementById("impact-results-overlay");
  if (overlay) {
    overlay.style.display = "none";
  }

  // Limpiar círculos en mapa 2D
  if (impactMap && destructionCircles.length > 0) {
    destructionCircles.forEach((circle) => impactMap.removeLayer(circle));
    destructionCircles = [];
  }

  // Limpiar círculos 3D
  if (earth && window.destructionCircles3D) {
    window.destructionCircles3D.forEach((circle) => earth.removeLayer(circle));
    window.destructionCircles3D = [];
  }

  // Limpiar marcador en mapa 2D
  if (impactMap && impactMarker) {
    impactMap.removeLayer(impactMarker);
    impactMarker = null;
  }

  // Limpiar marcador en globo 3D
  if (earth && impactMarker3D) {
    earth.removeMarker(impactMarker3D);
    impactMarker3D = null;
  }

  // Limpiar círculos en globo 3D
  if (earth && blastRadiusCircle) {
    earth.removeLayer(blastRadiusCircle);
    blastRadiusCircle = null;
  }

  // Ocultar panel de resultados en sidebar
  const resultsPanel = document.getElementById("results-panel");
  if (resultsPanel) {
    resultsPanel.style.display = "none";
  }

  console.log("Impact reset");
}
